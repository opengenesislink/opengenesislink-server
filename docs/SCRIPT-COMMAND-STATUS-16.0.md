# OpenGenesisLINK 16.0 — Script Command Status

Diese Datei wird aus den in 16.0 gepflegten ScriptEngine-Katalogen abgeleitet. Sie ist die Wiki-Referenz für den Implementierungsstand der nativen OGL-Skriptsprache und der LSL-Kompatibilität.

## Statusdefinition

| Status | Bedeutung |
|---|---|
| ✅ `implemented` | Im 16.0-Vertrag ausführbar und für den dokumentierten Umfang implementiert. |
...
| ⛔ `unsupported` | Derzeit absichtlich nicht unterstützt, veraltet oder nicht für den aktuellen Engine-Vertrag vorgesehen. |

**Wichtig:** Katalogabdeckung ist nicht dasselbe wie vollständige semantische LSL-Kompatibilität.

## Zusammenfassung

| Bereich | Gesamt | Implementiert | Partial | Recognized | Unsupported | Strikt implementiert | Ausführbar inkl. Partial |
|---|---:|---:|---:|---:|---:|---:|---:|
| OGL | 37 | 37 | 0 | 0 | 0 | 100.00% | 100.00% |
| LSL Funktionen | 523 | 62 | 34 | 402 | 25 | 11.85% | 18.36% |
| LSL Events | 44 | 1 | 13 | 29 | 1 | 2.27% | 31.82% |

16.0 erweitert die echte Event-Semantik insbesondere um automatischen State-Lifecycle, Collision/Land-Collision, authentifizierte Scene-Touch-Events sowie erste `changed`-Bitmasken für Scale- und Linkset-Änderungen.

Verifizierter Code-Checkpoint vor der Dokumentationsrunde: `9100b31a167a7e464145681cbd0d017366aff21f`.

## OGL-Befehle und Sprachmerkmale

OGL bleibt im aktuell definierten 16.0-Katalog bei 37/37 implementiert.
...
| `world.force` | physics | ✅ implemented |
| `world.impulse` | physics | ✅ implemented |
| `world.angular_impulse` | physics | ✅ implemented |
| `world.torque` | physics | ✅ implemented |
| `world.buoyancy` | physics | ✅ implemented |
| `world.material` | physics | ✅ implemented |
...
| Event | Kategorie | Status |
|---|---|---|
| `attach` | event | 🔵 recognized |
| `at_rot_target` | event | 🔵 recognized |
| `at_target` | event | 🔵 recognized |
| `changed` | event | 🟡 partial |
| `collision` | event | 🟡 partial |
| `collision_end` | event | 🟡 partial |
| `collision_start` | event | 🟡 partial |
...
| `land_collision` | event | 🟡 partial |
| `land_collision_end` | event | 🟡 partial |
| `land_collision_start` | event | 🟡 partial |
...
| `state_exit` | event | 🟡 partial |
| `timer` | event | ✅ implemented |
| `touch` | event | 🟡 partial |
| `touch_end` | event | 🟡 partial |
| `touch_start` | event | 🟡 partial |
| `transaction_result` | event | 🔵 recognized |

### In 16.0 automatisch erzeugte Eventpfade

- `state_entry` beim initialen Start und nach State-Wechsel
- `state_exit` vor State-Wechsel
- `collision_start`, `collision`, `collision_end`
- `land_collision_start`, `land_collision`, `land_collision_end`
- `touch_start`, `touch`, `touch_end` aus authentifizierter Scene Interaction
- `changed(CHANGED_SCALE)` mit Maske `0x008` bei tatsächlicher Scale-Änderung
- `changed(CHANGED_LINK)` mit Maske `0x020` bei Link/Unlink

Touch und Changed sind bewusst `partial`: Detected-Touch-Metadaten, alle CHANGED_* Varianten, Attach/Detach, Permissions, Inventory/Media und weitere vollständige Second-Life-Fidelity folgen später.
...
| `llApplyRotationalImpulse` | 🟡 partial |
| `llAsin` | ✅ implemented |
| `llAtan2` | ✅ implemented |
...
16.0 behauptet keine vollständige Second-Life-LSL-Kompatibilität. Insbesondere bleiben unter anderem Attach/Detach, Link Messages, Sensor/No-Sensor, Permissions, HTTP/DataServer, Inventory- und Animations-APIs sowie zahlreiche Viewer-/Parcel-/Media-Funktionen offen.

Die Prozentwerte werden direkt aus den vier Katalogzuständen berechnet. Ein `recognized`-Eintrag zählt nicht als ausführbar.
