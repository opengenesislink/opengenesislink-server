...
public:
 ...
 explicit ModerationStore(std::shared_ptr<storage::DatabasePool> database);
 void reload();
...
private:
 ...
 std::string path_; std::shared_ptr<storage::DatabasePool> database_; mutable std::mutex mutex_; std::unordered_map<std::string,BanRecord> bans_;
};
}
