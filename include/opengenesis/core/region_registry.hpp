...
struct RegionInfo {
    ...
    std::uint64_t node_generation{0}, ticks{0}, entities{0}, avatars{0}, physics_bodies{0};
    double sim_fps{0.0};
};
...
public:
    ...
    bool update_state(const std::string& id,const std::string& node_id,std::uint64_t generation,const std::string& state,std::string& reason);
...
private:
    ...
    std::string storage_path_; mutable std::mutex mutex_; std::unordered_map<std::string,RegionInfo> regions_;
};
}
