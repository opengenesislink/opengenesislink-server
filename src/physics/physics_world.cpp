...
void PhysicsWorld::step(const double dt) {
    ...
    for (auto& [_, body] : bodies_) {
        ...
        const double ground = ground_sampler_ ? ground_sampler_(body.position.x, body.position.y)
                                              : ground_height_;
        const double floor_z = ground + body.radius;
        if (body.position.z < floor_z) {
            body.position.z = floor_z;
            if (body.velocity.z < 0.0) {
                body.velocity.z = -body.velocity.z * std::clamp(body.restitution, 0.0, 1.0);
            }
...
void PhysicsWorld::set_ground_sampler(std::function<double(double, double)> sampler) {
    std::scoped_lock lock(mutex_);
